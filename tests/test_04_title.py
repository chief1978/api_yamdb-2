...
from .common import (
    auth_client, create_categories, create_genre, create_titles,
    create_users_api,
)
...
class Test04TitleAPI:
    ...
    def test_02_title_admin(self, admin_client):
        ...
        assert response.status_code == 400, (
            'Проверьте, что при POST запросе `/api/v1/titles/` с не '
            'правильными данными возвращает статус 400'
        )
        data = {
            'name': 'Поворот туда',
            'year': 2000,
        ...
        assert response.status_code == 201, (
            'Проверьте, что при POST запросе `/api/v1/titles/` с правильными '
            'данными возвращает статус 201'
        )
        data = {
            'name': 'Проект',
            'year': 2020,
            'genre': [genres[2]['slug']], 'category': categories[1]['slug'],
        ...
        if data['results'][0].get('name') == 'Поворот туда':
            ...
            assert False, (
                'Проверьте, что при GET запросе `/api/v1/titles/` '
                'возвращаете данные с пагинацией. '
                'Значение параметра `results` неправильное, `name` '
                'не найдено или не сохранилось при POST запросе.'
            )

        assert title.get('rating') is None, (
            'Проверьте, что при GET запросе `/api/v1/titles/` '
            'возвращаете данные с пагинацией. '
            'Значение параметра `results` неправильное, `rating` '
            'без отзывов должен быть равен `None`'
        )
        ...
        assert (
            genres[0] in title.get('genre', [])
            and genres[1] in title.get('genre', [])
        ), (
            'Проверьте, что при GET запросе `/api/v1/titles/` '
            'возвращаете данные с пагинацией. Значение параметра `results` '
            'неправильное, значение `genre` неправильное '
            'или не сохранилось при POST запросе.'
        )
        assert title.get('year') == 2000, (
            'Проверьте, что при GET запросе `/api/v1/titles/` возвращаете '
            'данные с пагинацией. Значение параметра `results` неправильное, '
