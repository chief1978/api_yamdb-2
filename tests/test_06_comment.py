...
class Test06CommentAPI:
    ...
    def test_02_comment(self, admin_client, admin):
        ...
        assert comment.get('author') == admin.username, (
            ...
            '`/api/v1/titles/{title_id}/reviews/{review_id}/comments/` '
            'возвращаете данные с пагинацией. '
            'Значение параметра `results` неправильное, `author` не '
            'найдено или не сохранилось при POST запросе.'
        )
        assert comment.get('pub_date'), (
            'Проверьте, что при GET запросе '
            '`/api/v1/titles/{title_id}/reviews/{review_id}/comments/`'
            ' возвращаете данные с пагинацией. Значение параметра '
            '`results` неправильное, `pub_date` не найдено.'
        )
    ...
    def test_03_review_detail(self, client, admin_client, admin):
        ...
        assert response.status_code != 404, (
            'Страница ''`/api/v1/titles/{title_id}'
