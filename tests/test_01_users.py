...
class Test01UserAPI:
    ...
    def test_04_01_users_get_admin_only(self, user_client):
        ...
        assert response.status_code == status, (
            f'Проверьте, что при GET запросе `{url}` не для администратора '
    ...
    def test_05_01_users_post_admin(self, admin_client, admin):
        ...
        assert response.status_code == 400, (
            'Проверьте, что при POST запросе `/api/v1/users/` '
            'без username, возвращаетe статус 400'
        )
        duplicate_email = {
    ...
    def test_07_01_users_username_patch_admin(self, admin_client, admin):
        ...
        assert response.status_code == 400, (
            'Проверьте, что при PATCH запросе `/api/v1/users/{username}/` '
            'от пользователя с ролью admin нельзя назначать произвольные роли '
            'пользователя и возвращается стастус 400'
        )
    ...
    def test_07_02_users_username_patch_moderator(
        ...
        assert response.status_code == 403, (
            'Проверьте, что при PATCH запросе `/api/v1/users/{username}/` '
            'пользователь с ролью moderator не '
            'может изменять данные других пользователей'
        )
    ...
    def test_07_03_users_username_patch_user(self, user_client, user):
        ...
        assert response.status_code == 403, (
            'Проверьте, что при PATCH запросе `/api/v1/users/{username}/` '
            'пользователь с ролью user не может изменять '
